...
function rlp_decode(input)
    ...
    elseif type == 'list' then
        ...
        while offset < #list_buf do
            ...
            -- print(_offset, _dataLen, _type, string.tohex(vbuf) )
            local v = rlp_decode(vbuf)
            -- print(_offset, _dataLen, _type, string.tohex(v) )
            -- print(v)
...
function decode_length(input, length)
    ...
    elseif prefix <= 0xb7 and length >= prefix - 0x80 then
        ...
        return 1, strLen, 'str'
    elseif prefix <= 0xbf and length > prefix - 0xb7 and length > prefix - 0xb7 +
        string.byte(string.sub(input, 2, prefix - 0xb7 + 1)) then
    ...
    elseif prefix <= 0xff and length > prefix - 0xf7 and length >= prefix - 0xf7 +
        string.byte(string.sub(input, 2, prefix - 0xf7 + 1)) then
...
function string.toip(hexstr)
    ...
    return string.byte(ipbytes, 1) .. "." .. string.byte(ipbytes, 2) .. "." .. string.byte(ipbytes, 3) .. "." ..
               string.byte(ipbytes, 4)
end
...
function devp2p.dissector(tvb, pinfo, tree)
    ...
    if types[ptype] == "PING" then
        ...
        local version, fromip, fromudpport, fromtcpport, toip, toport, expiration = string.match(decodedvalue,
            '%[([^,]+), %[([^,]+), ([^,]+), ([^%]]+)%], %[([^,]+), ([^%]]+)%], ([^%]]+)%]')
        print(version .. fromip .. fromudpport, fromtcpport, toip, toport, expiration)
        payloadtree:add("Version:", string.remove_quoted(version))
        payloadtree:add("From:", fromip, string.toport(string.remove_quoted(fromudpport)),
            string.toport(string.remove_quoted(fromtcpport)))
        payloadtree:add("To:",
        ...
        payloadtree:set_text("PING " .. string.toip(string.remove_quoted(toip)) .. ":" ..
                                 string.toport(string.remove_quoted(toport)))
    elseif types[ptype] == "PONG" then
        payloadtree:set_text(decodedvalue)
        local toip, toudpport, totcpport, replyhash, expiration = string.match(decodedvalue,
            '%[%[([^,]+), ([^,]+), ([^%]]+)%], ([^,]+), ([^%]]+)%]')
        payloadtree:add("To:", string.toip(string.remove_quoted(toip)), string.toport(string.remove_quoted(toudpport)),
            string.toport(string.remove_quoted(totcpport)))
        payloadtree:add("Hash:", string.remove_quoted(replyhash))
        payloadtree:add("Expiration:", string.remove_quoted(expiration))
        payloadtree:set_text("PONG " .. string.toip(string.remove_quoted(toip)) .. ":" ..
                                 string.toport(string.remove_quoted(toudpport)) .. "/" ..
                                 string.toport(string.remove_quoted(totcpport)))
